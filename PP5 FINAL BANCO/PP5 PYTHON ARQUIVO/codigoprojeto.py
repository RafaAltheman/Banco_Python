...
def salvar():
    ...
    for cliente in clientes:
        extrato_str = "".join(cliente[5])
        linha = f"[{cliente[0]}, {cliente[1]}, {cliente[2]}, {cliente[3]}, {cliente[4]}, {extrato_str}]\n"
        bancodedados.write(linha)
    bancodedados.close()
...
def extrato():
    ...
    for x in range(len(clientes)):
        if cnpj == clientes[x][1] and senha == clientes[x][4]:
            ...
            extrato_str = "".join(clientes[x][5])
            print("Extrato: ", extrato_str)
            return
    print("CNPJ ou senha inválido.")
                ...
                if cnpj2 == destinatario[1]:
                    ...
                    if remetente[2] == "comum":
                        ...
                        if valortotal <= -1000:
                            ...
                            remetente[5]+=("Data: " + str(now) + "  " + "-" + str(valortransf) + "    " + "Tarifa: " + str(tarifa) + "     " + "Saldo: " + str(valortotal))

                            valortotal_destinatario = float(destinatario[3]) + float(valortransf)
                            destinatario[3] = valortotal_destinatario
                            tarifa2 = 0.00
                            destinatario[5]+=("Data: " + str(now) + "  " + "+" + str(valortransf) + "    " + "Tarifa: " + str(tarifa2) + "     " + "Saldo: " + str(valortotal_destinatario))
